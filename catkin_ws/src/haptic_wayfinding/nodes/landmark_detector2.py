        ...
        while not rospy.is_shutdown():
            ...
            if ids is not None:
                ...
                for i in range(len(ids)):
                    ...
                    point_3d = rs.rs2_deproject_pixel_to_point(depth_intrin, [center_x, center_y], distance_z)
                    distance_x, distance_y, distance_z = point_3d

                    # Calculate the Euclidean distance
                    distance = np.sqrt(distance_x**2 + distance_y**2 + distance_z**2)

                    rospy.loginfo(f"Marker ID: {ids[i]}, Distance (3D): {distance}")
